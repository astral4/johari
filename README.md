...
Saved lists are written to `results-N.txt` (the first unused `N`) in the current directory.
...
Johari treats list-making as a Bayesian inference problem. Every character has a hidden score for how much you like them, and each answer you give is a noisy glimpse of those scores. Sessions start with a brief "bucket" stage where you sort each character into "meh", "fine", or "love", or skip anyone you don't recognize. The model interprets those marks as an ordinal probit and learns cutpoints as it goes. This gives it a rough picture of your tastes before any comparisons are shown.
